...
for idx, particle in enumerate(train_locations):
    ...
    # Zoom into the region around the particle
    zoom_min_x = max(0, x - zoom_factor * 20)
    zoom_max_x = min(class_mask.shape[1] - 1, x + zoom_factor * 20)
    zoom_min_y = max(0, y - zoom_factor * 20)
    zoom_max_y = min(class_mask.shape[0] - 1, y + zoom_factor * 20)
